...
var connection = new Connection(config);  
  ...
  router.post('/detail/:id',function(req,res){
      ...
      const email = req.body.email;
      let price = '';
      findFather = (cat,tempArray,thePostage,postageArr)=>{
        request = new Request(`select CATNAME as CATPrimary from CAT3withFather where RN='${tempArray[0].CATFatherRN}'  `, function(err,rowCounts,rows) {  
          ...
          if(rowCounts===0){
            ...
            cat[0]['thePostage'] = thePostage
            cat[0]['HunterBasicCharge'] = postageArr[0].HunterBasicCharge
            cat[0]['HunterMinCharge'] = postageArr[0].HunterMinCharge
            cat[0]['HunterRate'] = postageArr[0].HunterRate
            res.send(cat)
          }
          else{
            jsonArray=[]
      ...
      withCate = (cat, thePostage, postageArr)=>{
        ...
        if(rowCounts===0){
          ...
          for(var i=0; i < rowCounts; i++)
            ...
            findFather(cat,jsonArray,thePostage,postageArr);
      ...
      const withEstimate = (jsonArr, thePostage, postageArr) =>{
        ...
        where ${sentence} group by containersDetails.CustomLabel,containersDetails.SoldPerDay`,function(err,rowCounts,rows){
            ...
            for(var i=0; i < rowCounts; i++)
                ...
                withCate(productList, thePostage, postageArr)
            }}
        })
            request.on('row',function(columns){})
            request.on('requestCompleted', function () {
      ...
      //This line will print the array of JSON object.
      withPostage(jsonArray, thePostcode)
    })
    request.on('row',function(columns){})
    request.on('requestCompleted', function () {
      // Next SQL statement.
  ...
  withPostage = (jsonArr, thePostcode) =>{
  let totalWeight = jsonArr[0]['D WEIGHT']
  request = new Request(`select top 1 HunterBasicCharge,HunterMinCharge,HunterRate from HBAllCouriers where PostCode = ${thePostcode}`, function(err,rowCounts,rows) { 
    var jsonArray = []
    if(err){console.log(err);}
    else{
      for(var i=0; i < rowCounts; i++)
      ...
      if(jsonArray[0].HunterRate <= 0.23){
       withEstimate(jsonArr, 0, jsonArray)
      }else{
        ...
        if(TempostPrice > jsonArray[0].HunterMinCharge){
          ...
          withEstimate(jsonArr, TempostPrice, jsonArray)
        }
        else{
          let FinalMinCharge = parseInt(jsonArray[0].HunterMinCharge/5)*5+5
          withEstimate(jsonArr, FinalMinCharge, jsonArray)
        }
      }
    } 
 })
...
request.on('end',()=>{
  connection.close()
      ...
      request = new Request(`select * from WebCustomer where EmailAddress = '${email}'
      select * from WebCustomerAddress where Email = '${email}' and AddressType = 'R'`,function(err,rowCounts,rows){
        if(err){
          console.log(err);
        }else{
           if(rows.length > 0 ){
             ...
             for(var i=0; i < rowCounts; i++)
                 ...
                 withPrice(jsonArray[0].CustomerLevel, jsonArray[1].Postcode)
                //  console.log('a',jsonArray[0].CustomerLevel,  jsonArray[1].Postcode);
           }
           else{
             withPrice('0')
           }
